...
在「设置 → 向量嵌入」里把后端设为 `API 嵌入`，再填 Base URL 与模型名。**Base URL 要包含
`/v1`**——请求的是 `{base_url}/embeddings`，这是 OpenAI 约定；缺了这一段，多数服务会返回 404
（本项目脚本两种写法都能应答，但按约定填更稳）。真填错时接口会直接提示补 `/v1`，设置页的
「测试连接」也能当场验一次。
...
设置页会显示当前是否真的启用了向量腿、已嵌入多少块、以及有没有残留旧模型的向量（那些行不参与
检索，重建索引即可重算）。
