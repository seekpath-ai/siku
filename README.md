...
关键词检索（FTS5，中文按 bigram 索引）开箱即用，不需要任何配置。**语义召回是可选的一条附加腿**：
在「设置 → 语义搜索」里填入服务地址即启用，留空即关闭——关键词检索与语义召回按名次融合（RRF），
不是二选一。

点「测试连接」会自动读取服务声明的模型名（`GET /v1/models`）并填入模型名一栏，不必自己猜；服务没提供
这个接口时界面会说明，手填即可。

**服务地址要包含 `/v1`**——请求的是 `{base_url}/embeddings`，这是 OpenAI 约定；缺了这一段，多数
服务会返回 404（本项目脚本两种写法都能应答，但按约定填更稳）。真填错时接口会直接提示补 `/v1`，
设置页的「测试连接」可以用**当前填的值**当场试，不必先保存。
...
保存设置后，请在图书馆对文献右键执行「重建索引」生成向量。更换端点或模型名后，已有向量会按新模型
重算；端点暂时不可用时不会回退到占位向量——语义那条腿直接不参与召回，而不是拿无意义的数值去比对。

设置页会如实显示三种状态：未启用；已启用但还没有向量（提示去重建索引）；已启用且已生成 N/M 块。
早期版本用字符直方图占位实现凑出过一批假向量，它们已被改标为 `placeholder-hash` 并明确标注「不参与
检索」，不会再被误当成有效向量。
