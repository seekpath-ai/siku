...
pub async fn reprocess_paper_index(
    ...
    // The dual-pane paragraph cache derives from the same text layer —
    // recompute it on next open.
    sqlx::query("DELETE FROM paper_paragraphs WHERE paper_id = ?")
        .bind(&paper.id)
        .execute(&mut *tx)
        .await?;
...
pub async fn get_paragraphs(
    ...
    let rel_path = paper
        .file_path
        .as_deref()
        .ok_or_else(|| SikuError::NotFound("该文献没有关联的 PDF 文件".into()))?;
    let path = file_store::resolve_blob_path(app_data_dir, rel_path);
    if !path.exists() {
        return Err(SikuError::NotFound(format!("PDF 文件不存在: {path:?}")));
    }
    let paragraphs = pdf::extractor::extract_paragraphs(&path)?;
